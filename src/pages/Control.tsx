...
export function Control() {
 ...
 const listadePratos = useSelector((state: StoreState) => state.store);

 let getTotal = useSelector((state: StoreState) =>
  state.store.map((item) =>
   item.order.reduce(
    (total: number, itens) => (total += itens.amount * itens.price),
    0
   )
  )
 );
 ...
 if (getTotal.length > 0) {
  ...
  return (
   <>
    <ControlHeader
     text="Enviar pedido para cozinha"
     name="Comanda"
     show={true}
     idProduct={1}
     price={0}
   ...
   <ControlDiv>
    <ControlCards>
     {listadePratos.map((prato) => {
      return prato.order.map((orderItem, index) => {
       return (
        <Card
         key={index}
